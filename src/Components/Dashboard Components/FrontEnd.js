...
const FrontEnd = (props) => {
  ...
  return (
    <div className="container mt-5">
      <h1 className="display-4 text-dark mb-4">Front-End Developers</h1>
      <h5 className="text-secondary mb-4">
        List of candidates that applied for frontend Developer Job
      </h5>
      <table className="table table-striped table-borderless">
        ...
        <tbody>
          {data.map((d, id) => {
            if (d.jobTitle === "Front-End Developer") {
              return (
                <tr key={id}>
                  <td>{d.name}</td>
                  <td>{d.skills}</td>
